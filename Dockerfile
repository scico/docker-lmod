...
RUN  yum -y install epel-release && yum -y update && yum -y install git tar which bzip2 xz \
     ...
     iproute gcc gcc-c++ zlib-devel openssl-devel &&  yum clean all

RUN yum -y install sudo && useradd -u 1000 -d ${LMODDIR} apps && usermod -a -G wheel apps && sed -i -e '/wheel/s/#//g' /etc/sudoers
